...
def parse_args():
    ...
    parser.add_argument("--logging_steps", type=int, default=500, help="do pading")
    parser.add_argument("--save_steps", type=int, default=5000, help="do pading")
...
def main():
    ...
    def preprocess_function(examples):
        ...
        model_inputs = tokenizer(
            inputs, max_length=args.max_length, padding=padding, truncation=True
        )
    ...
    def postprocess_text(preds, labels):
        ...
        preds = [
            normalize_answer(pred.strip().replace("Agent :", "")) for pred in preds
        ]
        labels = [
            normalize_answer(label.strip().replace("Agent :", "")) for label in labels
        ]
    ...
    def evaluate_data(dataloader, eval_name="valid"):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                metric_bleu.add_batch(
                    predictions=_decoded_preds, references=_decoded_labels
                )
        ...
        if args.output_dir is not None:
            ...
            if accelerator.is_local_main_process:
                ...
                output_dir_file_name = os.path.join(
                    args.output_dir, f"{eval_name}-results.json"
                )
                print(output_dir_file_name)
                json.dump(
                    decoded_preds_extended, open(output_dir_file_name, "w"), indent=2
                )
